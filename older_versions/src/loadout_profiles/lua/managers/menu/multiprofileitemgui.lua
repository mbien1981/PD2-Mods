...
function MultiProfileItemGui:init(ws, panel)
	...
	self._profile_panel = self._profile_panel
		or self._panel:panel({
	...
	if managers.menu:is_pc_controller() then
		self._quick_select_panel = self._quick_select_panel
			or self._panel:panel({
	...
	if not arrow_left then
		if managers.menu:is_pc_controller() and not managers.menu:is_steam_controller() then
			arrow_left = self._profile_panel:bitmap({
				...
				color = mult:has_previous() and tweak_data.screen_colors.button_stage_3
					or tweak_data.menu.default_disabled_text_color,
			})
		else
			local BTN_TOP_L = managers.menu:is_steam_controller() and managers.localization:steam_btn("trigger_l")
				or managers.localization:get_default_macro("BTN_TOP_L")
	...
	if not arrow_right then
		if managers.menu:is_pc_controller() and not managers.menu:is_steam_controller() then
			arrow_right = self._profile_panel:bitmap({
				...
				color = mult:has_next() and tweak_data.screen_colors.button_stage_3
					or tweak_data.menu.default_disabled_text_color,
			})
		else
			local BTN_TOP_R = managers.menu:is_steam_controller() and managers.localization:steam_btn("trigger_r")
				or managers.localization:get_default_macro("BTN_TOP_R")
