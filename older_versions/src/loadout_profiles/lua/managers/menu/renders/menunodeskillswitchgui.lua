...
if not MenuNodeBaseGui.rec_round_object then
		...
		if unlocked then
			...
			distribution_text = distribution_text
				.. managers.localization:to_upper_text(
					points > 0 and "menu_st_points_unspent_skill_switch" or "menu_st_points_all_spent_skill_switch",
					{
		...
		elseif can_unlock then
			...
			for _, reason in ipairs(reasons) do
				...
				elseif reason == "level" then
					reasons_text = reasons_text
						.. managers.localization:to_upper_text("menu_st_req_level_skill_switch", {
							level = td.locks.level,
						})
				elseif reason == "achievement" then
					reasons_text = reasons_text
						.. managers.localization:to_upper_text("menu_st_req_achievement_skill_switch", {
							achievement = managers.localization:text("menu_st_achievement_" .. td.locks.achievement),
						})
				end
			end
			distribution_text =
				managers.localization:to_upper_text("menu_st_requires_skill_switch", { reasons = reasons_text })
			distribution_after_text = true
		end
...
function MenuNodeSkillSwitchGui:_align_marker(row_item)
	...
	if self.marker_color then
		self._marker_data.gradient:set_color(
			row_item.item:enabled() and self.marker_color or self.marker_disabled_color or row_item.disabled_color
		)
	end
